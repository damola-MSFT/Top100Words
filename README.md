...
TextReader (stdin / file / StringReader)
  ...
  ?
????????????????
?  WordParser   ? ?? Character-by-character extraction, ASCII letters and digits, lowercased
????????????????
  ?
  ?
??????????????????????
...
| `WordParser.cs` | Static helper yielding lowercase words from a line (ASCII letters and digits; punctuation and whitespace are delimiters) |
...
| File | Tests | Coverage |
|---|---|---|
| `WordParserTests.cs` | 10 | Empty, whitespace, single word, multi-word, punctuation, digits as part of words, mixed case, leading/trailing delimiters, tabs |
